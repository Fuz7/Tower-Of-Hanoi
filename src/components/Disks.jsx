...
export default function Disks({colorsArray,movesArray,isPlaying,scope,animate,setIsPlaying,playSpeed}){
  function delay(ms) {
      ...
      const animatePlaying = async(counter)=>{
       ...
       await delay(1000)
        ...
        if(counter !==movesArray.length){
          ...
          await animate(currentElement,{y: currentElementYValue +(-460 + (rodStartDiskNumber * 27)),},{duration:playingSpeed})
          ...
          await animate(currentElement,{x:(xValueWithPreviousTransformValue),},{duration:playingSpeed})
          const yValueCalculation = (-rodEndDiskNumber * 27) + (colorsArray.length - (currentElementIndex)) * 27
          await animate(currentElement,{y: 0  + yValueCalculation,},{duration:playingSpeed})
